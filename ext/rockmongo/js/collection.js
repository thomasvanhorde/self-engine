...
function showDataMenu(link, text) {
	...
	if (text.match(/^[\d\.]+$/)) {
		...
		if (text.length >= 10) {
			var date =  new Date();
			date.setTime(n * 1000);
			menu.push("ToDate: " + date.getFullYear() + "-" + (date.getMonth() + 1) + "-" + date.getDay() + " " + date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds() );
			width = 200;
		}
	}
	if (menu.length > 0) {
...
function switchDataType(div, type) {
	...
	switch(type) {
		...
		case "float":
			...
			div.find(".float_value").show();
			div.find(".mixed_value").hide();
			break;
		case "string":
			div.find(".value").show();
			div.find(".bool_value").hide();
			div.find(".float_value").hide();
			div.find(".mixed_value").hide();
			break;
		case "boolean":
			div.find(".value").hide();
...
function setValueWithData(div, data) {
	...
	switch(dataType) {
		case "integer":
		case "float":
			div.find("[name='float_value']").val(value);
			break;
		case "string":
...
function fieldOpQuery(link) {
	...
	fieldOpLoad(field, id, function (data) {
		...
		buttons["Query"] = function (){
			...
			$("#query_form").find("input[name='format']").val("array");
			$("#query_form").submit();
			$(this).dialog("close");
		}
		...
		buttons["Apply"] = function () {
			jQuery.ajax({
				data: { 
					...
					"float_value":div.find("[name='float_value']").val(),
...
function showQueryHistory() {
	...
	jQuery.ajax({
		...
		"dataType": "html",
		"url": "index.php?action=index.queryHistory",
